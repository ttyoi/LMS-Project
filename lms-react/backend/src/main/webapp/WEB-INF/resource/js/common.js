 ...
 * if (document.documentMode) {
  // IE인 경우 true
  alert("Internet Explorer 감지됨");
}

 * $.browser = {};  // 빈 객체 생성

(function(){
    jQuery.browser.msie = false;  // 기본값: IE가 아님
    $.browser.version = 0;        // 버전 초기화

    // 브라우저가 IE일 경우 감지
    if (navigator.userAgent.match(/MSIE ([0-9]+)\./)) {
        $.browser.msie = true;                    // IE 브라우저임
        jQuery.browser.version = RegExp.$1;       // IE 버전 번호 추출
    }
})(); * 
 */
