...
<script>
    ...
    function loadTestList() {
        ...
        $.ajax({
            url: "/inst/exams/list",   // ※ ITestController 교사용 URL
            type: "GET",
            data: {
    ...
    function renderList() {
        ...
        testList.forEach((t, idx) => {
            ...
            const scoreText = t.date == null ? "--" : t.score;
            const grade = getGradeText(t.score);

            let evalHtml = "";

            if (t.score == null) {
                evalHtml = `<button class="action-btn btn-ghost small" disabled>미응시</button>`;
            } else {
                evalHtml = `
                    <button class="action-btn btn-primary small" onclick="viewResult(${t.courseId},${t.period},'${t.studentId}')">결과확인</button>
    ...
    // 결과 확인 이동
    function viewResult(courseId,period,studentId) {
        console.log(courseId,period,studentId);
        window.location.href = `/inst/exams/detail/${courseId}/${period}/${studentId}`;
    }
    ...
    // 필터 변경
    document
        .querySelector("#wrap_area #container .contents .content .container #courseFilter")
        .addEventListener("change", function () {
            currentPage = 1;
            loadTestList();
        });

    // 초기 로드 => jsp 로 변경에 따라 자동으로 DOM 로드 후 script 실행
